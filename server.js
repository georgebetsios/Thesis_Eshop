...
const express = require('express');//Web framework για τη δημιουργία server.
const mongoose = require('mongoose');//ORM για σύνδεση και διαχείριση MongoDB.
const session = require('express-session');//Χρήση sessions για έλεγχο σύνδεσης χρηστών.
const pdf = require('pdfkit');//Δημιουργία αρχείων PDF (χρήσιμο για αποδείξεις πληρωμών).
const fs = require('fs');//Διαχείριση αρχείων στο filesystem (χρησιμοποιείται για να αποθηκεύει PDF).
const xlsx = require('xlsx');//Χρήσιμο για ανάγνωση αρχείων Excel και εισαγωγή δεδομένων στη βάση.
const nodemailer = require('nodemailer');//Αποστολή email (χρήσιμο για αποστολή αποδείξεων πληρωμών).


const User = require('./user.model'); //Εισάγει το μοντέλο χρήστη από το αρχείο user.model.js.

const app = express();//Αρχικοποιεί μια Express εφαρμογή.*******

app.use(express.static('./'));//****** 
app.use(express.json());//Επιτρέπει στο Express να διαβάζει JSON data από HTTP αιτήματα./
app.use(
  session({
    secret: 'your-secret-key',// Ένα μυστικό κλειδί που χρησιμοποιείται για την κρυπτογράφηση των sessions.
    resave: false,//Αποτρέπει την αποθήκευση του session αν δεν υπάρχουν αλλαγές. (Βελτιώνει την απόδοση).
    saveUninitialized: false//Δεν αποθηκεύει ένα session αν δεν έχει δεδομένα (Εξοικονομεί μνήμη).
  })
);


var logged_in_User;//Δημιουργεί μια global μεταβλητή για να κρατάει τον συνδεδεμένο χρήστη.
...
function readAndParseExcel() {
  const workbook = xlsx.readFile('./Database.xlsx');//Ανοίγει το Database.xlsx
  const sheetName = workbook.SheetNames[0];//Διαβάζει το πρώτο φύλλο εργασίας
  const worksheet = workbook.Sheets[sheetName];//Παίρνει το περιεχόμενο του επιλεγμένου φύλλου εργασίας.
  const data = xlsx.utils.sheet_to_json(worksheet, { raw: true });//Μετατρέπει τα δεδομένα σε JSON μορφή
